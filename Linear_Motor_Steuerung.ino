...
#define ARRAYSIZE sizeof(pwmSin)/sizeof(int)
...
double position_in_mm = 0; // (encoder0Pos) / 40.00;
String inString = "";

void setup() 
{
  commutationPWMOffset = ARRAYSIZE/3;
...
void drive(double scale_factor)
{
  if(scale_factor < 0.00)
  {
    ...
    int offset = -(phasePwmIndex[0] - berechne() + commutationPWMOffset);
    shift(&pos_U, offset, ARRAYSIZE, PHASE1);
    shift(&pos_V, offset, ARRAYSIZE, PHASE2);
    shift(&pos_W, offset, ARRAYSIZE, PHASE3);
